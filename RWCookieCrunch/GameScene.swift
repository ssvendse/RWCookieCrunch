...
//        }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if success {
            if let cookie = level.cookieAt(column: column, row: row) {
                ...
                swipeFromRow = row
                showSelectionIndicatorForCookie(for: cookie)
            }
        }
    }
    ...
    func trySwap(horizontal horzData: Int, vertical vertDelta: Int) {
        let toColumn = swipeFromColumn! + horzData
    ...
    func showSelectionIndicatorForCookie(for cookie: Cookie) {
